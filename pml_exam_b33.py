...
sigma_delta = torch.tensor(sigma_var, dtype=torch.float64, device=device)
...
if __name__ == "__main__":
    ...
    samples,arviz_data = sample_likelihood(init_samples, 1000, num_mc, 0.01)
    print("MCMC done. Arviz summary:", arviz.summary(arviz_data))

    flat_samples = samples.reshape(-1, len(delta_i))
    idx = np.random.choice(flat_samples.shape[0], size=num_mc, replace=False)

    mu_list = []
    var_list = []
    for i in idx:
        delta_s = flat_samples[i]
        mu_s, var_s = conditional_fixed(delta_s)
        mu_list.append(mu_s)
        var_list.append(var_s) 
    mu_stack = torch.stack(mu_list)    # (S, n)
    var_stack = torch.stack(var_list)  # (S, n)
    mu_marginal = mu_stack.mean(dim=0)
    var_marginal = var_stack.mean(dim=0) + mu_stack.var(dim=0)
    print("Marginal variances:", var_marginal.detach().cpu().numpy())
    lower = mu_marginal - 1.96 * torch.sqrt(var_marginal)
    upper = mu_marginal + 1.96 * torch.sqrt(var_marginal)
    print("mean est.", mu_marginal.detach().cpu().numpy())
    print("std est.", torch.sqrt(var_marginal).detach().cpu().numpy())
    ...
    plt.close()
    # Plot marginal posterior predictive
    x_star_np = X.squeeze().cpu().numpy()
    plt.figure()
    plt.plot(x_star_np, mu_marginal.detach().cpu().numpy(), label="Posterior Mean", color='blue')
    plt.plot(x_star_np, f(x_star_np), label="Ground Truth", color='green')
    plt.fill_between(x_star_np, lower.detach().cpu().numpy(), upper.detach().cpu().numpy(), alpha=0.3, color='blue', label="95% CI")
    # Plot data
    plt.scatter(x_i, y_i, color='red', label="Data")
    plt.title("GP Posterior with Input Noise Uncertainty (Marginalized)")
    plt.legend()
    plt.savefig("pyro_posterior_marginal.png")
    plt.close()